<x-app-layout>
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">
            @lang('site.create') @lang('site.dashboard.orders')
        </h1>
            ...
            @csrf
                ...
                <select name="service_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($services as $option)
                        <option value="{{ $option->id }}" {{ old('service_id') == $option->id ? 'selected' : '' }}>{{ $option->name }}</option>
                    @endforeach
                </select>
                @error('service_id')
                ...
                <input type="text" name="project_type" value="{{ old('project_type') }}" class="w-full border border-gray-300 rounded p-2">
                @error('project_type')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
